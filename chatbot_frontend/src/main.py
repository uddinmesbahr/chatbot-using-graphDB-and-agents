...
with st.sidebar:
    ...
    st.markdown(
        """
        This chatbot interfaces with a multiple agents to answer the question about operations, delivery, customer reviews.
        The agent uses  retrieval-augment generation (RAG) over both
        structured and unstructured data that has been synthetically generated. This also gives graphical visualtion of any data if wanted
        as like Buisiness Intellegent Reports. 
        """
    )

    st.header("Example Questions")
    st.markdown("- How much sales in 2021 ?")
    st.markdown(
        """- shows me the sales over the year for customer no 01"""
    )
    # st.markdown(
...
st.info(
    """Ask me questions about products, review, deleivery, stock or operation relate questions!"""
)

# Add options to select before asking a question
option = st.selectbox("Do you want a Visual Report or Query?", ("Query", "Visual Report"))
